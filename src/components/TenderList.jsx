  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-10 sm:p-14 text-center">
      ...
      <p className="text-slate-500 text-sm mt-1.5 max-w-sm mx-auto">
        Try widening the value range, clearing the location filter, or searching
        a broader keyword.
      </p>
    </div>
  );
...
function TenderList({ tenders }) {
  if (!tenders?.length) return <EmptyState />;

  return (
    <div className="space-y-3">
      {tenders.map((tender, i) => (
        <TenderCard key={tender._id ?? i} tender={tender} index={i} />
      ))}
    </div>
  );
}
